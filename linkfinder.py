#!/usr/bin/env python
# Python 3
# LinkFinder with Banner
# By Gerben_Javado (modified to include banner)
...
# 定义彩色方案
LINE_COLOR = Fore.CYAN  # 分割线颜色
TEXT_COLOR = Fore.GREEN  # 文本信息颜色
ART_COLORS = [
    Fore.RED,
    Fore.YELLOW,
    Fore.GREEN,
...
# 新增输出颜色定义
LINK_COLOR = Fore.GREEN       # 链接输出颜色
ERROR_COLOR = Fore.RED        # 错误信息颜色
STATUS_COLOR = Fore.YELLOW      # 状态信息颜色
INFO_COLOR = Fore.YELLOW      # 提示信息颜色
OUTPUT_COLOR = Fore.CYAN      # 输出文件信息颜色

# ASCII图案
ascii_art = [
    r" _     _       _    _____ _           _          ",
    r"| |   (_)_ __ | | _|  ___(_)_ __   __| | ___ _ __ ",
    r"| |   | | '_ \| |/ / |_  | | '_ \ / _` |/ _ \ '__|",
    r"| |___| | | | |   <|  _| | | | | | (_| |  __/ |   ",
    r"|_____|_|_| |_|_|\_\_|   |_|_| |_|\__,_|\___|_|   "
]


def print_centered(text, width=80, color=Fore.WHITE):
    """居中打印文本"""
    if len(text) >= width:
        print(color + text)
        return
    indent = (width - len(text)) // 2
    print(" " * indent + color + text)


def print_colorful_art(art, colors):
    """彩色打印ASCII艺术字并优化布局"""
    max_len = max(len(line) for line in art)
    term_width = 80  # 终端宽度
    indent = (term_width - max_len) // 2  # 计算居中缩进
...
def show_banner():
    ...
    # 上方分割线
    print(LINE_COLOR + separator)
    print()  # 空行

    # 打印ASCII艺术
    print_colorful_art(ascii_art, ART_COLORS)
    print()  # 空行

    # 作者信息
    print_centered("LinkFinder Tool", term_width, TEXT_COLOR)
    print_centered("Author: Bifishone", term_width, TEXT_COLOR)
    print_centered("GitHub: https://github.com/Bifishone", term_width, TEXT_COLOR)
    print()  # 空行
...
def parser_error(errmsg):
    ...
    Error Messages
    '''
    print(INFO_COLOR + "Usage: python %s [Options] use -h for help" % sys.argv[0])
    print(ERROR_COLOR + "Error: %s" % errmsg)  # 错误信息红色
    sys.exit()
...
def cli_output(endpoints):
    ...
    for endpoint in endpoints:
        # 链接输出为绿色
        print(LINK_COLOR + html.escape(endpoint["link"]).encode(
...
def html_save(html):
    ...
    try:
        ...
        # 输出文件信息为青色
        print(OUTPUT_COLOR + "URL to access output: file://%s" % os.path.abspath(args.output))
        ...
        # 错误信息为红色
        print(ERROR_COLOR + "Output can't be saved in %s \
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input",
                        ...
                        For folders s a wildcard can be used (e.g. '/*.js').",
        ...
        if args.domain:
            ...
            for endpoint in endpoints:
                ...
                if endpoint is False:
                    continue
                # 状态信息为黄色
                print(STATUS_COLOR + "Running against: " + endpoint)
                print(STATUS_COLOR + "")
                try:
                    ...
                    # 错误信息为红色
                    print(ERROR_COLOR + "Invalid input defined or SSL error for: " + endpoint)
                    continue
